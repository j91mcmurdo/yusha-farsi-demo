...
import { FileText, MoreVertical, Pencil, Trash2, Volume2, Loader2 } from 'lucide-react';
...
export default function WordCard(props: WordCardProps) {
  const { id, farsi, finglish, english, category, categoryName, tagNames, notes, audioUrl, recordingId, status, showAdminControls = false, showStatusBadge = false } = props;
  ...
  const handleEdit = () => {
    ...
    openAdminDialog('word', null, 'edit', { ...props, tags: props.tagNames });
  };
  ...
  const playAudio = async () => {
    ...
    // Otherwise, we need to generate/fetch it.
    setIsGeneratingAudio(true);
    try {
        const result = await getOrCreateAudioAction({ docId: id, text: farsi, recordingId, audioUrl });
        setCurrentAudioUrl(result.audioUrl); // Cache the URL in component state
  ...
  return (
    <Card className="h-full flex flex-col bg-primary/20 border-primary/40 shadow-md hover:shadow-lg transition-shadow duration-300 relative">
      <CardHeader className="pt-6 pb-4">
        ...
        <div className="flex justify-between items-start">
            <CardTitle className={cn("text-4xl font-headline font-bold text-foreground", showAdminControls && 'pt-8')} dir="rtl">
                {farsi}
            </CardTitle>
